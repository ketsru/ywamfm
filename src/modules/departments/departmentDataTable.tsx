...
export function DepartmentDataTable() {
  ...
  // ── Query ─────────────────────────────────────────────────
  const { data, isLoading, isError } = useQuery<Department[], Error, Department[]>({
    queryKey: ["admin", "departments"],
    // map API DTOs to our Department type (ensure image is string)
    queryFn: () =>
      getAllDepartments().then((dtos) =>
        dtos.map((dto) => ({ ...dto, image: dto.image ?? "" })) as Department[]
      ),
  });
